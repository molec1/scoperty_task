...
from amenities import augment_by_amenities, top_amenities, \
    get_amentities, amen_cols
...
filtered_df = project_coordinates(filtered_df)

regions_df = make_regions_df(filtered_df, min_leaf=100)
...
We know nothing about house type and state
(material, condition, number of floors)
So the model is very basic
total_price = land_meter_price*total_area/number_of_units
            + house_meter_price*living_area
...
reg = ElasticNet(positive=True, alpha=0.02).\
    fit(X_train[predictors], y_train)
...
def get_model_score(reg, X_test):
    ...
    dt = (
        X_test[['prediction', 'price', 'err', 'err_prcnt',
                'abs_err', 'abs_err_prcnt']]
        .describe()
        .T
    )
    return dt
